...
import { Upload, FileText, CheckCircle2, RotateCw, Trash2, Scissors, Copy, Plus, Crop, Bookmark, Shield, Sliders } from "lucide-react";
...
export default function ArchetypeB({
  ...
  // 3. Active Workspace (Rule 0 Contained Viewport Container)
  return (
    <div className="w-full bg-[#FFFFFF] border-[0.5px] border-[#EFE1D2] rounded-[12px] overflow-hidden my-2 flex flex-col max-h-[calc(100vh-140px)]">
      ...
      <div className="bg-[#FBF1E9] border-b [border-bottom-width:0.5px] border-[#EFE1D2] px-4 py-3 flex flex-wrap items-center justify-between gap-3 shrink-0">
        ...
        <button
          ...
          className="px-5 py-2 bg-[#E8792A] hover:bg-[#D66B1E] disabled:opacity-50 text-white text-sm font-medium rounded-lg transition-colors flex items-center gap-2 shrink-0"
        >
          {isProcessing ? "Processing..." : `${toolName}`}
        </button>
      </div>

      {/* Main Grid & Slim Sidebar Panel */}
      <div className="flex flex-col md:flex-row flex-1 overflow-hidden">
        
        {/* Left Bounded Page-Thumbnail Grid (Internal Scroll Only) */}
        <div className="flex-1 overflow-y-auto p-4 max-h-[55vh] md:max-h-none">
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
            {pages.map((p, idx) => (
              <div key={p.id} className="relative group">
                ...
                >
                  {/* Thumbnail Number & Checkbox Overlay */}
                  ...
                  {/* Thumbnail Visual Placeholder / DataURL */}
                  {p.dataUrl ? (
                    <img src={p.dataUrl} alt={`Page ${p.pageNumber}`} className="w-full h-full object-contain my-1 rounded" />
                  ) : (
                    <div className="w-full flex-1 border border-dashed border-[#EFE1D2] rounded flex items-center justify-center text-[10px] text-[#9C9488]">
                      Page {p.pageNumber}
                    </div>
                  )}
                  ...
                  {toolSlug === "rotate-pdf" && onRotatePage && (
                    <button
                      ...
                      className="absolute bottom-2 right-2 p-1 bg-white border-[0.5px] border-[#EFE1D2] hover:border-[#E8792A] text-[#262B36] rounded-md transition-colors"
                      title="Rotate Page"
                    >
                ...
                {/* Add Blank Page (+) Insertion Point */}
                {toolSlug === "add-blank-page" && onAddBlankPageAt && (
                  <button
                    onClick={() => onAddBlankPageAt(idx + 1)}
                    className="absolute -right-2 top-1/2 -translate-y-1/2 z-10 w-5 h-5 bg-[#E8792A] hover:bg-[#D66B1E] text-white rounded-full flex items-center justify-center text-xs shadow-sm"
                ...
                {/* Split Marker Indicator */}
                {toolSlug === "split-pdf" && p.isSplitPoint && (
                  <div className="absolute -right-2 top-0 bottom-0 w-1 bg-[#E8792A] rounded-full" title="Split Marker" />
                )}
              </div>
            ))}
          </div>
        ...
        {/* Right Slim Tool Settings Sidebar */}
        <div className="w-full md:w-64 bg-[#FBF1E9]/50 border-t md:border-t-0 md:border-l [border-left-width:0.5px] border-[#EFE1D2] p-4 flex flex-col gap-4 shrink-0 overflow-y-auto">
          <div className="text-xs font-medium text-[#262B36] border-b [border-bottom-width:0.5px] border-[#EFE1D2] pb-2">
            {toolName} Options
          </div>

          {toolSlug === "rotate-pdf" && onRotateAll && (
            <div className="space-y-2">
              <label className="block text-xs text-[#9C9488]">Rotate All Pages</label>
              <div className="grid grid-cols-3 gap-1">
                <button onClick={() => onRotateAll(90)} className="py-1.5 border-[0.5px] border-[#EFE1D2] bg-white text-xs text-[#262B36] rounded hover:border-[#E8792A]">
                  90°
                </button>
                <button onClick={() => onRotateAll(180)} className="py-1.5 border-[0.5px] border-[#EFE1D2] bg-white text-xs text-[#262B36] rounded hover:border-[#E8792A]">
                  180°
                </button>
                <button onClick={() => onRotateAll(270)} className="py-1.5 border-[0.5px] border-[#EFE1D2] bg-white text-xs text-[#262B36] rounded hover:border-[#E8792A]">
                  270°
                </button>
              </div>
            </div>
          ...
          {toolSlug === "split-pdf" && (
            <div className="space-y-3">
              <div>
                <label className="block text-xs font-medium text-[#262B36] mb-1">Split Range</label>
                <input
                  type="text"
                  value={rangeInput}
                  onChange={(e) => setRangeInput(e.target.value)}
                  placeholder="e.g. 1-3, 5, 7-9"
                  className="w-full text-xs text-[#262B36] bg-white border-[0.5px] border-[#EFE1D2] rounded px-2.5 py-2 focus:outline-none focus:border-[#E8792A]"
                />
              </div>
            </div>
          )}
          ...
          {toolSlug === "crop-pdf" && (
            <div className="space-y-2 text-xs">
              <label className="block font-medium text-[#262B36]">Crop Margins (%)</label>
              <div className="grid grid-cols-2 gap-2">
                ...
                <div>
                  <span className="text-[#9C9488]">Top</span>
                  <input type="number" value={cropTop} onChange={(e) => setCropTop(Number(e.target.value))} className="w-full bg-white border-[0.5px] border-[#EFE1D2] rounded p-1" />
                </div>
                <div>
                  <span className="text-[#9C9488]">Bottom</span>
                  <input type="number" value={cropBottom} onChange={(e) => setCropBottom(Number(e.target.value))} className="w-full bg-white border-[0.5px] border-[#EFE1D2] rounded p-1" />
                </div>
              </div>
            </div>
          )}
          ...
          {(toolSlug === "delete-pages" || toolSlug === "extract-pages") && (
            <div className="bg-white p-2.5 rounded-lg border-[0.5px] border-[#EFE1D2] text-xs text-[#262B36]">
              Selected: <strong className="text-[#E8792A]">{selectedCount}</strong> pages
            </div>
          )}

          <div className="mt-auto bg-white p-2.5 rounded-lg border-[0.5px] border-[#EFE1D2] text-center">
